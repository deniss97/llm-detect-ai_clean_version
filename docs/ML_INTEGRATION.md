...
Текущий веб-сервис уже реализует нужный сценарий:
...
После этого в веб-проекте достаточно добавить ещё одну реализацию протокола `Detector`, которая вызывает этот ML-сервис по HTTP. Такой вариант чище для ВКР: веб-сервис отвечает за загрузку, OCR, БД и UI, а ML-сервис - за тяжёлую модель и её зависимости.

## Важное ограничение

Модель из отчёта обучалась на англоязычном Kaggle-датасете. Если OCR распознаёт русскоязычные тексты, качество может быть ниже. Для честного результата в ВКР стоит явно написать, что текущий ML-блок требует либо перевода текста перед детекцией, либо дообучения/валидации на русскоязычных данных.
